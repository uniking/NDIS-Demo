    ...
    else do
    {
        ...
        if (Packet != NULL)
        {
            ...
            if (Status == NDIS_STATUS_SUCCESS)
            {
				...
				myStatus = PacketAnalysis(pAdapt, pAdapt->MiniportHandle, MyPacket, RECEIVE_PACKET);
				if (myStatus == FORBID_NET_WORK)
				{
					return NDIS_STATUS_FAILURE;
				}
				else if (myStatus == STORAGE_PACKET)
				{
					//return NDIS_STATUS_PENDING;
					return NDIS_STATUS_NOT_ACCEPTED;
				}
				else if (myStatus == RECEIVE_STORAGE_PACKET)
				{
...
--*/
{
    ...
    if (Remaining)
    {
		...
		myStatus = PacketAnalysis(pAdapt ,pAdapt->MiniportHandle, Packet, RECEIVE_PACKET);
		if (myStatus == FORBID_NET_WORK)
		{
			return NDIS_STATUS_FAILURE;
		}
