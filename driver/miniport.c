...
--*/
{
    ...
    //
    {
        ...
        if (Remaining)
        {
			...
			PacketAnalysis(pAdapt, pAdapt->BindingHandle, Packet, SEND_PACKET);
            NdisSend(&Status,
    ...
    if (Status == NDIS_STATUS_SUCCESS)
    {
		...
		if (PacketAnalysis(pAdapt, pAdapt->BindingHandle, MyPacket, SEND_PACKET) == FORBID_NET_WORK)
		{
			return NDIS_STATUS_FAILURE;
		}
