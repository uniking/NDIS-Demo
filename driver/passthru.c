...
{
    NDIS_STATUS                        Status;
    NDIS_PROTOCOL_CHARACTERISTICS      PChars;//协议特征
    NDIS_MINIPORT_CHARACTERISTICS      MChars;//小端口特征
    NDIS_STRING                        Name;
...
--*/
{
    ...
    switch (irpStack->MajorFunction)
    {
        ...
        case IRP_MJ_DEVICE_CONTROL:
			if (pDeviceExtension->iDeviceType == 1221)
			{
				status = DDKDeviceIOControl(DeviceObject, Irp);
			}
            //
